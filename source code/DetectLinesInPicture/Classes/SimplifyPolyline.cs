...
namespace DetectLinesInPicture {
    internal static class SimplifyPolyline {
        ...
        public static DIntB[] Reduce(DIntB[] points, double tolerance) {
            ...
            // Z nejakého duvodo blbne
           // if (points[0] == points[points.Length-1]) return points;
            ...
            //unsafe {
            //    fixed (DIntB* ptr = points, result = returnPoints) {
            //        for (int i = 0; i < l; ++i) *(result + i) = *(ptr + indexesToKeep[i]);
            //    }
            //}
           ...
           // unsafe {
            ...
            for (int i=0; i<l; i++){ 
               returnPoints[i] = points[indexesToKeep[i]];
            }
        ...
        //    unsafe {
        //        fixed (DIntB* samples = points) {
        //            DIntB point1 = *(samples + firstIndex);
        //            DIntB point2 = *(samples + lastIndex);
